...
def create_app(test_config=None):
  ...
  class User(UserMixin, db.Model): #UserMixin provides authentication defaults, db.Model makes this class into an ORM
    __tablename__ = 'users'
  ...
  def signUp():
    ...
    db.session.add(User(email=email, password_hash=hash, cannonical_email=email.lower()))
    db.session.commit()
    return '200'
